        ...
        public static void Main()
        {
            ...
            while (true)
            {
                using (var stream = listener.AcceptTcpClient().GetStream())
                {
                    ...
                    switch (url)
                    {
                        case "/": resource = "../../index.html"; break;
                        case "/info": resource = "../../info.html"; break;
                        default:
                            hasError = true;
                            resource = "../../error.html";
                            break;
                    ...
                    if (!hasError)
                    {
                        responseHeader = Encoding.UTF8.GetBytes(string.Format("HTTP/1.1 200 OK Content-Type: text/html\r\n\r\n"));
                    }
                    else
                    { 
                        responseHeader = Encoding.UTF8.GetBytes(string.Format("HTTP/1.1 404 Not Found Content-Type: text/html\r\n\r\n"));
                    }
